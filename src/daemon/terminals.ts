...
import type { DaemonCreateParams } from '../shared/daemon'
import type {
  TerminalDataEvent,
  TerminalExitEvent,
  TerminalInfo,
...
/**
 * Owns every PTY. Lives in the detached daemon process so sessions survive
 * IDE restarts (docs/plans/pty-daemon.md); the Electron main process talks
 * to it through the socket server in ./server.ts. Env for each terminal
 * arrives fully computed from the client — the daemon adds only the
 * per-terminal binding vars.
 */
export class TerminalManager extends EventEmitter {
  private terminals = new Map<string, ManagedTerminal>()

  create(options: DaemonCreateParams): TerminalInfo {
